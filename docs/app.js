...
// ===== Bootstrapping and event binding =====
...
function bindEvents() {
  ...
  settingsAddBtn.addEventListener("click", () => openEditor());
  ...
  categoryPresetSelect.addEventListener("change", syncCategoryInputFromPreset);
  categoryInput.addEventListener("input", syncCategoryPresetFromInput);
  ...
  deleteBtn.addEventListener("click", deleteEditingRestaurant);
  form.addEventListener("submit", saveEditorForm);
}

function syncCategoryInputFromPreset() {
  if (categoryPresetSelect.value) {
    categoryInput.value = categoryPresetSelect.value;
  }
}

function syncCategoryPresetFromInput() {
...
function deleteEditingRestaurant() {
  if (!editingId) return;
...
// ===== Storage, normalization, and conflict merging =====
function loadState() {
...
// ===== Data sanitizers and shared state helpers =====
...
// ===== Dialogs, import/export, and link parsing =====
...
// ===== Page navigation and editor choice controls =====
...
// ===== Roulette canvas and wheel interaction =====
...
// ===== Recent history, data charts, and cleanup =====
